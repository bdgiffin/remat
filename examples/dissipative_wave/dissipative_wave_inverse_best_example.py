...
# -----------------------------------------------------------------------------
# Element-wise best-example configuration
# -----------------------------------------------------------------------------
...
IMPACT_CENTERS = [1.0, 2.5, 3.5, 5.0]
...
EARLY_STOP_MIN_EVALS = 80
EARLY_STOP_DATA_LOSS = 1.0e-4
...
def make_true_stiffness(problem):
    y = np.asarray(problem["elem_centers"], dtype=np.double)[:, 1]
    labels = np.zeros(y.size, dtype=np.int32)
    labels[y >= 0.5 * HEIGHT] = 1
    stiffness = np.where(labels == 0, TRUE_BOTTOM_STIFFNESS, TRUE_TOP_STIFFNESS).astype(np.double)
    return stiffness, labels
...
def invert_elementwise(experiments, observations, obs_norm_sq, tau_elem):
    nelem = experiments[0]["connectivity"].shape[0]
    i_idx, j_idx = inv.build_edge_pairs(NX, NY)
    x0 = np.full(nelem, INIT_STIFFNESS, dtype=np.double)
    ...
    def objective_with_grad(x):
        ...
        if len(history) >= EARLY_STOP_MIN_EVALS and data_loss <= EARLY_STOP_DATA_LOSS:
            state["status"] = "early_stop_data_loss"
            raise EarlyStop()
...
def save_summary_plot(problem, true_stiffness, recovered_stiffness, observed_history, recovered_history):
    ...
    panels = [
        ...
        (axes[0, 1], recovered_stiffness, "Recovered stiffness"),
        (axes[1, 0], np.abs(recovered_stiffness - true_stiffness), "Absolute stiffness error"),
    ]

    vmin = float(min(np.min(true_stiffness), np.min(recovered_stiffness)))
    vmax = float(max(np.max(true_stiffness), np.max(recovered_stiffness)))
    for ax, field, title in panels:
        ...
        im = ax.imshow(
            ...
            vmax=float(np.max(field)) if is_error else vmax,
        )
    ...
    fig.tight_layout()
    fig.savefig(OUTPUT_PLOT, dpi=200, metadata={"Title": "Element-wise dissipative wave inverse best example"})
    fig.clf()
    plt.close(fig)
...
def main():
    ...
    nelem = experiments[0]["connectivity"].shape[0]
    true_stiffness, labels = make_true_stiffness(experiments[0])
    tau_elem = np.full(nelem, FIXED_TAU, dtype=np.double)

    print("Generating synthetic multi-impact observations...")
    ...
    result = invert_elementwise(experiments, observations, obs_norm_sq, tau_elem)
    ...
    layer_summary = []
    for label, name, truth in [
        (0, "bottom", TRUE_BOTTOM_STIFFNESS),
        (1, "top", TRUE_TOP_STIFFNESS),
    ]:
        values = recovered_stiffness[labels == label]
        layer_summary.append(
            {
                "name": name,
                "true_stiffness": float(truth),
                "mean_recovered_stiffness": float(np.mean(values)),
                "mean_absolute_error": float(np.mean(np.abs(values - truth))),
                "max_absolute_error": float(np.max(np.abs(values - truth))),
            }
        )
    ...
    payload = {
        "config": {
            ...
            "optimizer": {
                ...
                "early_stop_data_loss": EARLY_STOP_DATA_LOSS,
        ...
        "fields": {
            ...
            "initial_stiffness": np.full(nelem, INIT_STIFFNESS, dtype=np.double).tolist(),
    ...
    for row in layer_summary:
        print(
            f"  {row['name']:>6s} layer mean  : true={row['true_stiffness']:.6f}, "
